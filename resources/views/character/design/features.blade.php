...
@if($request->status == 'Draft' && $request->user_id == Auth::user()->id)
    ...
    {!! Form::open(['url' => 'designs/'.$request->id.'/traits']) !!}
        ...
        <div class="form-group">
            ...
            @if($request->character->is_geno_slot && $request->character->image->subtype_id)
        ...
        <div class="form-group">
            ...
            @if($request->character->is_geno_slot && $request->character->image->rarity_id)
        ...
        <div class="form-group">
            {!! Form::label('Traits') !!}
            <div id="featureList">
                {{-- Add in the compulsory traits for geno slots --}}
                @if($request->character->is_geno_slot && $request->character->image->features)
...
@else
    ...
    <h5>Traits</h5>
    <div>
        @if($request->character && $request->character->is_geno_slot && $request->character->image->features)
            @foreach($request->character->image->features as $feature)
